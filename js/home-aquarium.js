(function(){
  ...
  var def={lang:{x:8,y:36},book:{x:420,y:72},article:{x:24,y:322},chapter:{x:986,y:318},quakes:{x:980,y:48},fish:{x:-150,y:196},clown:{x:820,y:282},violet:{x:240,y:314},green:{x:610,y:238},pawn:{x:748,y:364}};
  ...
  var posts=[{el:lang,key:'lang'},{el:book,key:'book'},{el:article,key:'article'},{el:chapter,key:'chapter'},{el:quakes,key:'quakes'},{el:fish,key:'fish'}];
  ...
  function shouldUseAquariumDefaults(loaded){return !hasSavedPositions()||!loaded||!loaded.lang||!loaded.book||!loaded.article||!loaded.chapter||!loaded.quakes||samePos(loaded.lang,legacyDef.lang)&&samePos(loaded.book,legacyDef.book)&&samePos(loaded.quakes,legacyDef.quakes);}
  function mediumLayout(){var pad=6,gap=22;var langPos={x:pad,y:38};var articleX=pad+14;var langHeight=lang?lang.offsetHeight:248;var articleWidth=(article?article.offsetWidth:260)+14;var leftWidth=Math.max(lang?lang.offsetWidth:270,articleWidth);var rightWidth=Math.max(book?book.offsetWidth:260,quakes?quakes.offsetWidth:270,chapter?chapter.offsetWidth:260);var rightX=Math.max(pad+leftWidth+28,Math.round(bounds.w-rightWidth-pad));var bookY=72;var bookHeight=book?book.offsetHeight:320;var quakesY=bookY+bookHeight+gap;var quakesHeight=quakes?quakes.offsetHeight:248;return {lang:langPos,book:{x:rightX,y:bookY},article:{x:articleX,y:langPos.y+langHeight+gap},quakes:{x:rightX,y:quakesY},chapter:{x:rightX+6,y:quakesY+quakesHeight+gap}};}
  function desktopLayout(){var pad=6,gap=22;var langPos={x:pad,y:38};var langHeight=lang?lang.offsetHeight:248;var articleY=langPos.y+langHeight+gap;var quakesX=Math.max(pad,Math.round(bounds.w-278));var quakesY=52;var quakesHeight=quakes?quakes.offsetHeight:248;var bookX=Math.max(pad+300,Math.min(quakesX-292,Math.round(bounds.w*0.49)-130));return {lang:langPos,book:{x:bookX,y:72},article:{x:pad+14,y:articleY},chapter:{x:quakesX+6,y:quakesY+quakesHeight+18},quakes:{x:quakesX,y:quakesY}};}
